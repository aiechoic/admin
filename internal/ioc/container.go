...
type Providers[T any] struct {
	...
	fn func(name string) *Provider[T]
	mu sync.Mutex
}
...
func NewProviders[T any](fn func(name string) *Provider[T]) *Providers[T] {
	return &Providers[T]{
		ps: map[string]*Provider[T]{},
		fn: fn,
	}
...
func (ps *Providers[T]) GetProvider(name string) *Provider[T] {
	ps.mu.Lock()
	defer ps.mu.Unlock()
	p, ok := ps.ps[name]
	if !ok {
