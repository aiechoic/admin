...
// GormProviders defines a providers for gorm.DB, it can be redefined
var GormProviders = ioc.NewProviders(func(name string) *ioc.Provider[*CloseAbleGormDB] {
	return ioc.NewProvider(func(c *ioc.Container) (*CloseAbleGormDB, error) {
		...
		vp, err := GetViper(name, GormInitConfig, c)
		if err != nil {
			return nil, err
		}
		var cfg config
...
func GetGormDB(name string, c *ioc.Container, models ...any) (*gorm.DB, error) {
	db, err := GormProviders.GetProvider(name).Get(c)
	if err != nil {
		return nil, err
	}
	if db.autoMigrate {
...
func GetDefaultGormDB(c *ioc.Container, models ...any) (*gorm.DB, error) {
	return GetGormDB(DefaultGormConfig, c, models...)
}
