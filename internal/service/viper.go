...
// viperAdapterProvider is a provider for ViperAdapter, which is a factory for creating Viper instances.
var viperAdapterProvider = ioc.NewProvider(func(c *ioc.Container) (ViperAdapter, error) {
	return NewLocalViperAdapter("configs", Testing), nil
})

// SetViperAdapter is a helper function to set the ViperAdapterProvider.
func SetViperAdapter(adapter ViperAdapter) {
	viperAdapterProvider = ioc.NewProvider(func(c *ioc.Container) (ViperAdapter, error) {
		return adapter, nil
	})
}
...
func GetViper(name string, initConfig string, c *ioc.Container) (*viper.Viper, error) {
	viperAdapter, err := viperAdapterProvider.Get(c)
	if err != nil {
		return nil, err
	}
...
// ViperAdapter is a factory for creating Viper instances.
type ViperAdapter interface {
...
// LocalViperAdapter is a ViperAdapter implementation that stores configuration files locally.
type LocalViperAdapter struct {
	dir string
	env Env
}

func NewLocalViperAdapter(dir string, env Env) *LocalViperAdapter {
	...
	return &LocalViperAdapter{
		dir: dir,
		env: env,
	}
}

// NewViper implements the ViperAdapter interface. If the configuration file does not exist, it creates a new
...
func (l *LocalViperAdapter) NewViper(name, ext, initConfig string) (*viper.Viper, error) {
