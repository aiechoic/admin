...
const DefaultRedisConfig = "redis"
...
// RedisProviders defined providers for redis client, it can be redefined
var RedisProviders = ioc.NewProviders(func(name string) *ioc.Provider[*redis.Client] {
	return ioc.NewProvider(func(c *ioc.Container) (*redis.Client, error) {
		...
		vp, err := GetViper(name, RedisInitConfig, c)
		if err != nil {
			return nil, err
		}
...
func GetRedisClient(name string, c *ioc.Container) (*redis.Client, error) {
	return RedisProviders.GetProvider(name).Get(c)
}

func GetDefaultRedisClient(c *ioc.Container) (*redis.Client, error) {
	return GetRedisClient(DefaultRedisConfig, c)
}
